...
defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class JobClassificationModel extends CI_Model {
		...
		public function LoadJobClassificationMasterlist($id = null) {
			$this->db->select('c.*,u.FirstName as ModFirstName,u.LastName as ModLastName');
			$this->db->from(' tbl_applicants_jobclassification c');
		...
		public function Add($data) {
			...
			$this->db->insert('tbl_applicants_jobclassification');

			$id = $this->db->insert_id();

			if ($id > 0) {
				$inserted = $this->LoadJobClassificationMasterlist($id);
				return $inserted;
			}
			else {
				return FALSE;
		...
		public function Delete($data) {
			...
			$this->db->update('tbl_applicants_jobclassification');
		...
		public function Update($id, $data) {
		    ...
		    $this->db->set('IsActive', '"'.$data['status'].'"', FALSE);
		    $this->db->where('Id', $id);
		    $query = $this->db->update('tbl_applicants_jobclassification');
			$update = $this->db->affected_rows();
			if ($update > 0) {
				$result = $this->LoadJobClassificationMasterlist($id);
				return $result;
			}
			else {
				return FALSE;
