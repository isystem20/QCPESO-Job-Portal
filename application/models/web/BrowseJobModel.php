...
defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class BrowseJobModel extends CI_Model {
		...
		public function  BrowseJobModelMasterlist($data = null, $id = null, $userid = null) {
			$this->db->select('ej.* ,"" as CategList, "" as SkillReq,estab.CompanyName,apposition.Name app_position,industry.Name industryname,dress.Name dresscode,appemtype.Name apptype,appemlevel.Description applevel,(select count(Id) from tbl_applicants_job_applications where JobPostId = ej.Id and ApplicantId = "'.$userid.'" ) as AppliedJob, aja.IsActive as ajaIsActive, (select IsActive from tbl_applicants_job_applications where JobPostId = ej.Id and ApplicantId = "'.$userid.'" ) as ajaStatus, estab.WhyJoinUs as estabWhyJoinUs,"'.$userid.'" as currentuser');
