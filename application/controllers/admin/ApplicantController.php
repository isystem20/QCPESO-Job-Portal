 ...
 class ApplicantController extends Admin_Controller {
    ...
    public function Applicantlist()
    {
        ...
        $layout = array('tables'=>TRUE, 'datepicker'=>TRUE, 'pagetitle'=>'Applicantlist');
        $data['masterlist'] = $this->applimod->LoadMasterlist();
        $data['class'] = 'applicantlist';
        $this->load->view('layout/admin/1_css',$layout);
        $this->load->view('layout/admin/2_preloader',$layout);
        $this->load->view('layout/admin/3_topbar',$layout);
        $this->load->view('layout/admin/4_leftsidebar',$layout);
        $this->load->view('pages/transaction/applicants/Applicantlist',$data);
        $this->load->view('layout/admin/6_js',$layout);     
        $this->load->view('layout/admin/7_modals',$layout); 

        $json = json_encode($data['masterlist']); //log
        $this->logger->log('Load Masterlist','Applicantlist',$json); //Log 
    }
    ...
    public function Delete() {
        ...
        if ($this->form_validation->run() == FALSE){
            $errors = validation_errors();
            $this->logger->log('Error Form Create','Applicantlist',$errors); //Log
            echo json_encode(['error'=>$errors]);
        }
        else{
            ...
            if ($result != FALSE) {
                $json = json_encode($result);              
               $this->logger->log('Delete','Applicantlist',$json); //Log
                echo $json;
            }
            else {
                $json = json_encode($postdata); // encode postdata
                $this->logger->log('Error Delete','Applicantlist',$json); //Log 
                echo json_encode(['error'=>'Update Unsuccessful.']);
            }
