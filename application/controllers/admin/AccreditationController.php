 ...
 class AccreditationController extends Admin_Controller {
    ...
    public function AccreditationRequest()
    {
        ...
        $layout = array('tables'=>TRUE, 'datepicker'=>TRUE, 'pagetitle'=>'Accreditation Masterlist');
        ...
        $this->logger->log('Load AccreditationRequest','AccreditationRequest',$json); //Log 
 	 ...
 	 public function Create() {
		 $this->form_validation->set_rules('name','Name','required|is_unique[tbl_applicants_languages.name]',
		     ...
		     if ($this->form_validation->run() == FALSE){
              ...
              $this->logger->log('Error Form Create','AccreditationRequest',$errors); //LoggerModel
         	...
         	$inserted = $this->establishmentmod->Add($postdata);
          	...
          	if ($inserted != FALSE) {
         		...
         		$this->logger->log('Create','AccreditationRequest',$json); //Log  
                
            echo $json;
          }
          else {
                $json = json_encode($postdata); // encode postdata
                $this->logger->log('Error Create','AccreditationRequest',$json); //Log 
 	 ...
 	 public function Update() {
         ...
         if ($this->form_validation->run() == FALSE){
             $errors = validation_errors();
             $this->logger->log('Error Form Create','AccreditationRequest',$errors); //Log
             echo json_encode(['error'=>$errors]);
         }
         else{
             $id = $postdata['itemid'];
             ...
             if ($result != FALSE) {
                 ...
                 $this->logger->log('Update','AccreditationRequest',$json); //Log            
                 echo $json;
             }
             else {
                  $json = json_encode($postdata); // encode postdata
                  $this->logger->log('Error Update','AccreditationRequest',$json); //Log  
                  echo json_encode(['error'=>'Update Unsuccessful.']);
             }
         }
 	...
 	public function Delete() {
        ...
        if ($this->form_validation->run() == FALSE){
            ...
            if ($result != FALSE) {
                $json = json_encode($result);
                $this->logger->log('Delete','AccreditationRequest',$json); //Log              
                echo $json;
            }
            else {
                $json = json_encode($postdata); // encode postdata
                $this->logger->log('Error Delete','AccreditationRequest',$json); //Log 
                echo json_encode(['error'=>'Update Unsuccessful.']);
            }
