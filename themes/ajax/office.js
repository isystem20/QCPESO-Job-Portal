...
$('#add-btn1').click(function(e){ //Input the form's ID or CLASS, use # for ID and . for CLASS
    $('#add-modal1').modal();
  });

$('#add-form1').submit(function(e){ //Input the form's ID or CLASS, use # for ID and . for CLASS
    e.preventDefault();       //This prevents the action to move to other page.
        $("#add-submit1").prop("disabled", true);   //Disables the submit button after click 
        var newURL = $(this).attr('action');      //Get the form action attribute value.
        var newData  = {
                'name' : $('input[name=name]').val(),     //List of data you want to post
                'eligibiltytitle' : $('input[name=eligibiltytitle]').val(),
                'description' : $('textarea[name=description]').val(),
                'status' : $('select[name=status]').val(),
            }
          $.ajax({
              ...
              success: function(data) {
                ...
                if($.isEmptyObject(data.error)){      //Checking if the data.error has value
                    ...
                    var id = data[0].id;
                    var name = data[0].name;
                    var eligibiltytitle = data[0].eligibilityTitle;
                    var desc = data[0].description.substr(0,30);
                    var modby = data[0].modifiedById;
                    // var modat = $.datepicker.formatDate('yy-dd-mm', new Date(data[0].modifiedAt));
                    var modat = data[0].modifiedAt;
                    ...
                    var row = table.row.add( [
                      name,eligibiltytitle,desc,modby,modat,status,actions,
                      ]).draw().node();
                ...
                $("#add-submit1").prop("disabled", false);     //Reenable the submit button after the action           
              }
          });   
  });
